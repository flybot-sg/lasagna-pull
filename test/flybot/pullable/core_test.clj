...
(deftest fn-query
  (testing "fn-query select key from a value"
    (are [args data exp] (= exp ((apply sut/fn-query args) data))
      [:a] {:a 1 :b 2}   {:a 1}
      [:a] {:b 2}        {}))
  (testing "fn-query throws data error if data is not a map (associative)"
    (is (thrown? ExceptionInfo ((sut/fn-query :a) 3)))))

(deftest join-query
  (are [data exp] (= exp ((sut/join-query (sut/fn-query :a) (sut/fn-query :b)) data))
    {:a {:b 1 :c 2}}    {:a {:b 1}}
    {:c 3}              {}
    ))

(deftest filter-query
  (are [data exp] (= exp ((sut/filter-query (sut/fn-query :a) #(= % 3)) data))
    {:a 3}   {}
    {:a 1}   nil))
...
(deftest vector-query
  ...
  (testing "a single child is equivlent to itself"
    (is (= {:a 1} ((sut/vector-query [(sut/fn-query :a)]) {:a 1}))))
...
(deftest post-process-query
  (is (= {:a 3} ((sut/post-process-query (sut/fn-query :a) (constantly [:a 3])) {:a 1}))))

(deftest seq-query
  (testing "seq query queries by apply q on every item"
    (are [q data exp] (= exp ((sut/seq-query q) data))
  ...
  (testing "seq query throws data error on non-sequential data"
    (is (thrown? ExceptionInfo ((sut/seq-query (sut/fn-query :a)) {:a 1})))))

(deftest mk-named-var-query
  (let [bindings (fn [sym-table status]
                   (let [t-sym-table (transient sym-table)
                         a-status (atom status)]
                     [(((sut/mk-named-var-query t-sym-table a-status 'a) (sut/fn-query :a)) {:a 1 :b 2})
